    ...
    try:
        ...
        for result in results[:10]:
            try:
                ...
                research_papers.append({"title": title, "link": link, "snippet": snippet, "authors": authors_text, "year": year, "source": "Google Scholar"})
            except:
                continue
...
def scrape_arxiv(query):
    ...
    for entry in root.findall("{http://www.w3.org/2005/Atom}entry"):
        ...
        research_papers.append({"title": title, "link": link, "snippet": snippet, "authors": authors, "year": year, "source": "arXiv"})
    ...
    try:
        ...
        for result in results[:5]:
            try:
                ...
                research_papers.append({"title": title, "link": link, "snippet": snippet, "authors": authors_text, "year": year, "source": "IEEE Xplore"})
            except:
                continue
...
def scrape_semantic_scholar(query):
    ...
    for paper in data.get("data", []):
        research_papers.append({"title": paper["title"], "link": paper["url"], "snippet": paper.get("abstract", "No abstract available"), "authors": ", ".join([author["name"] for author in paper.get("authors", [])]), "year": paper.get("year", 0), "source": "Semantic Scholar"})
...
def fetch_top_research_papers(query):
    ...
    sorted_papers = sorted(all_papers, key=lambda x: x["year"], reverse=True)
